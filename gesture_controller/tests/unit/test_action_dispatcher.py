...
def dummy_config() -> MagicMock:
    config = MagicMock()
    config.get.side_effect = lambda key, default=None: {"profiles.auto_detect_app": True}.get(
        key, default
    )
    return config
...
def test_action_dispatcher_routing_os(
    mock_controller: MagicMock, dummy_config: MagicMock, event_bus: EventBus
) -> None:
...
def test_action_dispatcher_routing_keypress(
    mock_controller: MagicMock, dummy_config: MagicMock, event_bus: EventBus
) -> None:
    dispatcher = ActionDispatcher(mock_controller, dummy_config, event_bus)
...
def test_action_dispatcher_routing_scroll(
    mock_controller: MagicMock, dummy_config: MagicMock, event_bus: EventBus
) -> None:
    dispatcher = ActionDispatcher(mock_controller, dummy_config, event_bus)
...
def test_action_dispatcher_routing_media(
    mock_controller: MagicMock, dummy_config: MagicMock, event_bus: EventBus
) -> None:
    dispatcher = ActionDispatcher(mock_controller, dummy_config, event_bus)
...
def test_action_dispatcher_app_profile_resolution(
    mock_controller: MagicMock, dummy_config: MagicMock, event_bus: EventBus
) -> None:
