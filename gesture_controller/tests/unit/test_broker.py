...
def test_broker_server_client_flow(temp_broker_address: str) -> None:
    ...
    try:
        ...
        with (
            patch(
                "gesture_controller.os_integration.broker.get_broker_address",
                return_value=temp_broker_address,
            ),
            patch(
                "gesture_controller.os_integration.broker.get_broker_family",
                return_value=server.family,
            ),
        ):
