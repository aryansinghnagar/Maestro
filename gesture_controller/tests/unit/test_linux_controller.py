...
def test_linux_key_press_xdotool_fallback(
    mock_run: MagicMock,
...
def test_linux_get_foreground_app_sway(
    ...
    mock_res.stdout = (
        '{"focused": false, "nodes": [{"focused": true, "type": "con", "app_id": "Firefox"}]}'
    )
...
def test_linux_minimize_active_window(
    ...
    mock_run.assert_called_once_with(
        ["swaymsg", "[focused] move scratchpad"], capture_output=True, timeout=2
    )
...
@patch("os.write")
@patch("subprocess.run")
def test_linux_media_keys_playerctl(
    mock_run: MagicMock, mock_write: MagicMock, mock_open: MagicMock, mock_system: MagicMock
) -> None:
...
@patch("shutil.which", return_value="/usr/bin/xdotool")
@patch("subprocess.run")
def test_linux_key_release_uinput_and_xdotool(
    mock_run: MagicMock,
    mock_which: MagicMock,
    mock_write: MagicMock,
    mock_open: MagicMock,
...
@patch("shutil.which", return_value="/usr/bin/xdotool")
@patch("subprocess.run")
def test_linux_mouse_double_click(
    mock_run: MagicMock,
    mock_which: MagicMock,
    mock_write: MagicMock,
    mock_open: MagicMock,
...
@patch("shutil.which", return_value="/usr/bin/xdotool")
@patch("subprocess.run")
def test_linux_mouse_scroll_xdotool(
    mock_run: MagicMock,
    mock_which: MagicMock,
    mock_write: MagicMock,
    mock_open: MagicMock,
...
@patch("shutil.which", return_value="/usr/bin/xdotool")
@patch("subprocess.run")
def test_linux_get_foreground_app_xdotool(
    mock_run: MagicMock,
    mock_which: MagicMock,
    mock_write: MagicMock,
    mock_open: MagicMock,
