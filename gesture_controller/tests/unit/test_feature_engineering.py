...
def test_feature_engineering_open_palm(open_palm_hand: Hand) -> None:
    ...
    features = compute_features(open_palm_hand, velocity, acceleration, timestamp=0.0, frame_number=0)
...
def test_feature_engineering_pointing(pointing_hand: Hand) -> None:
    velocity = np.zeros((21, 3))
    acceleration = np.zeros((21, 3))
    features = compute_features(pointing_hand, velocity, acceleration, timestamp=0.0, frame_number=0)
...
def test_scale_invariance(open_palm_hand: Hand) -> None:
    ...
    features1 = compute_features(open_palm_hand, velocity, acceleration, timestamp=0.0, frame_number=0)
    
    # Scale coordinates by 2x
    scaled_landmarks = tuple(
        Landmark3D(x=lm.x * 2.0, y=lm.y * 2.0, z=lm.z * 2.0)
        for lm in open_palm_hand.landmarks
    )
    scaled_hand = Hand(
...
def test_hand_mirroring_left_to_right(open_palm_hand: Hand) -> None:
    ...
    features_right = compute_features(open_palm_hand, velocity, acceleration, timestamp=0.0, frame_number=0)
    features_left = compute_features(left_hand, velocity, acceleration, timestamp=0.0, frame_number=0)
