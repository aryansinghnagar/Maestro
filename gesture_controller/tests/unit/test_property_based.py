...
from gesture_controller.core.state_machine import (
    GestureFSM, FSMState, FSMTransition, compile_condition
)
...
@given(
    s1=arrays(np.float64, (5, 3), elements=st.floats(min_value=-10.0, max_value=10.0, allow_nan=False, allow_infinity=False)),
    s2=arrays(np.float64, (6, 3), elements=st.floats(min_value=-10.0, max_value=10.0, allow_nan=False, allow_infinity=False))
)
...
def test_feature_invariance(landmarks: list[Landmark3D], dx: float, dy: float, dz: float, scale: float) -> None:
    # 1. Original hand
...
# -----------------------------------------------------------------------------
@given(
    index_ext=st.booleans(),
    index_curl_val=st.floats(min_value=0.0, max_value=1.0)
)
def test_fsm_never_single_frame_trigger(index_ext: bool, index_curl_val: float) -> None:
    ...
    fv = FeatureVector(
        thumb_extended=False, index_extended=index_ext, middle_extended=False, ring_extended=False, pinky_extended=False,
        thumb_curl=0.0, index_curl=index_curl_val, middle_curl=0.0, ring_curl=0.0, pinky_curl=0.0,
