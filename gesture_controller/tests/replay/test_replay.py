...
def fsm_manager(config_manager: ConfigManager) -> GestureFSMManager:
    ...
    pinch_fsm = {
        ...
        "states": [
            {
                "id": "Idle",
                "transitions": [{"to": "Pinching", "condition": "pinch_distance < 0.15"}],
            },
            {
                "id": "Pinching",
                "min_duration_ms": 150,
            ...
            {"id": "Trigger", "action": "MouseClick:Left", "cooldown_ms": 500},
        ],
    }
...
def test_replay_minimize(fsm_manager: GestureFSMManager) -> None:
    ...
    for frame_idx, f in enumerate(frames):
        ts = f["timestamp"]
        hand_data = f["hands"][0]
        lms = tuple(Landmark3D(x=lm["x"], y=lm["y"], z=lm["z"]) for lm in hand_data["landmarks"])
