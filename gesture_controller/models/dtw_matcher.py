...
@numba.jit(nopython=True)
def dtw_distance_batch(query: np.ndarray, templates: np.ndarray, thresholds: np.ndarray) -> tuple[int, float]:
...
def to_hand_frame(landmarks, handedness: str) -> list[Landmark3D]:
...
class CustomGestureMatcher:
    ...
    def __init__(self, config: dict | None = None) -> None:
        ...
        if sys_name == "Windows":
            self._template_dir = Path(os.environ.get("APPDATA", "")) / "gesture_controller" / "custom_templates"
        elif sys_name == "Darwin":
            self._template_dir = Path.home() / "Library" / "Application Support" / "gesture_controller" / "custom_templates"
        else:
    ...
    def update_buffer(self, hand: Hand) -> None:
        ...
        flat = np.array([l.x for l in normalized] + [l.y for l in normalized] + [l.z for l in normalized], dtype=np.float64)
