...
def compute_features(
    ...
    norm_mag = float(np.sqrt(palm_norm_x*palm_norm_x + palm_norm_y*palm_norm_y + palm_norm_z*palm_norm_z))
    if norm_mag < 1e-8:
        palm_normal = np.array([0.0, 0.0, 1.0], dtype=np.float64)
    else:
        palm_normal = np.array([palm_norm_x / norm_mag, palm_norm_y / norm_mag, palm_norm_z / norm_mag], dtype=np.float64)
